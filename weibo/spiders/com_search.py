            ...
            if not self.settings.get('REGION') or '全部' in self.settings.get(
                ...
                if not isinstance(self.start_time, list):
                    ...
                    if self.start_time == '0' and self.end_time == '0':
                        start_str = start_date_str + '-0'
                        end_str = end_date_str + '-0'
                ...
                # 按照多时间段爬取
                    ...
                    for date_str in util.iter_date(start_date_str, end_date_str, day_gap=1):
                        for i in range(len(self.start_time)):
                            start_str = date_str + '-' + self.start_time[i]
                            end_str = date_str + '-' + self.end_time[i]
                            target_url = url + '&timescope=custom:{}:{}'.format(start_str, end_str)
                            yield scrapy.Request(url=target_url,
                                                 callback=self.parse,
                                                 meta={
            ...
            if self.meta_only:
                ...
                for page in range(2, page_count + 1):
                    yield scrapy.Request(url=response.url + '&page=' + str(page),
                                         ...
                                         meta={'keyword': keyword}, priority=1)  # 优化爬取顺序
            else:
    ...
    def parse_page(self, response):
        ...
        if is_empty:
            ...
            if self.meta_only:
                ...
                # self.parse_weibo(response)  # 错误！只有yield Request直接作用的函数里的yield才会被当作item处理！
                for weibo in self.parse_weibo(response):
                    self.check_environment()
                    yield weibo
                next_url = response.xpath(
    ...
    def parse_meta(self, response):
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                meta_item['keyword'] = keyword
                # print(meta_item)
                # self.logger.log(msg=meta_item, level=logging.INFO)
                yield meta_item
        # try:
        #     self.logger.log(msg=response.url.split(':')[-1], level=logging.INFO)
        # except Exception as e:
        #     self.logger.log(msg=response.url, level=logging.INFO)
